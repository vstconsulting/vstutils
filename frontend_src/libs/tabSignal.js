 ...
 */
tabSignal.emit = function(
  signal_name,
  param,
  SignalNotFromThisTab = false,
  failed = false
) {
  if (tabSignal.slotArray[signal_name] === undefined) {
    if (tabSignal.debug)
  ...
  try {
    if (window["localStorage"] !== undefined) {
      var curent_custom_id =
        ...
        "_" +
        Math.random();
      window["localStorage"]["tabSignal_storage_emit"] = JSON.stringify({
        name: signal_name,
        custom_id: curent_custom_id,
        param: param
      });
    }
    return true;
