...
import { deepEqual, addCssClassesToElement, trim, isEmptyObject } from '../utils';
...
const base_field_description_mixin = {
    ...
    computed: {
        description() {
            return this.$t((this.field.options.description || this.field.options.help || '').toLowerCase());
        },
    },
};
        ...
        parseCronString(string) {
            ...
            for (let key in this.crontab_elements) {
                if (this.crontab_elements.hasOwnProperty(key)) {
                    ...
                    this.parseItem(this.model[el.name], this.model[el.name + 'Str'], el.start, el.end);
                }
            }
        },
...
const main_pagination_mixin = {
    ...
    methods: {
        ...
        hideItemOrNot(number) {
            return Math.abs(number - this.current_page) > 2 && number > 3 && this.pages_amount - number > 3;
        },
    },
};
...
const gui_fields_mixins = {
    ...
    autocomplete: {
        components: {
            field_content_edit: {
                mixins: [base_field_content_edit_mixin, autocomplete_field_content_edit_mixin],
            },
        },
    },
    ...
    date_time: {
        components: {
            field_content_readonly: {
                mixins: [base_field_content_readonly_mixin, date_time_field_content_mixin],
            },
            field_content_edit: {
    ...
    inner_api_object: {
        ...
        components: {
            field_content_readonly: {
                mixins: [base_field_content_readonly_mixin, field_inner_api_object_content_mixin],
                template: '#template_field_content_readonly_inner_api_object',
            },
            field_content_edit: {
                mixins: [base_field_content_edit_mixin, field_inner_api_object_content_mixin],
...
export { mixins, gui_fields_mixins };
