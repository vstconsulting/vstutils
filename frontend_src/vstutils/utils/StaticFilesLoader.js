...
// createDomElement and StaticFilesLoader just copied from app-loader.js
// beacause it required by ./TestsFilesLoader. Possible solution is:
// TODO move this to to separate file that will be globally included before app-loader.js
 ...
 */
export default class StaticFilesLoader {
  ...
  checkAllFilesLoaded(response) {
    for (let index in response) {
      ...
      if (item.status != 200) {
        throw new Error(
          `${item.status} error occurred during file loading - "${item.url}"`
        );
      }
    }
