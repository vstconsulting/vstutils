 ...
 */
export const the_basest_view_mixin = {
  data() {
    return {
      /**
...
export const editable_page_mixin = {
  methods: {
    ...
    setQuerySetInSandBox(view, url) {
      ...
      try {
        page_view = this.$store.getters.getViews[
          view.schema.path.replace("/edit", "")
        ];
      } catch (e) {}
    ...
    getQuerySetFromSandBox(view, url) {
      let qs = this.$store.getters.getQuerySetFromSandBox(
        url.replace(/^\/|\/$/g, "")
      );

      if (!qs) {
        this.setQuerySetInSandBox(view, url);
        return this.$store.getters.getQuerySetFromSandBox(
          url.replace(/^\/|\/$/g, "")
        );
      }
...
export let routesComponentsTemplates = {
  ...
  base: {
    ...
    computed: {
      ...
      breadcrumbs: function() {
        ...
        return breadcrumb.concat(
          this.getBreadcrumbParentPart(),
          this.getBreadcrumbCurrentPath()
        );
      },
      /**
       * Current URL of view.
    ...
    methods: {
        ...
        let paths = Object.values(app.views)
          .filter(item => {
            if (
              ...
              item.schema.path.replace(/^\/|\/$/g, "").split("/").last ==
                "{" + pk_key + "}"
            ) {
              return item;
            }
        ...
        qs.formQueryAndSend(method)
          .then(response => {
            ...
            if (response && response.data) {
              try {
                let redirect_path = this._getRedirectUrlFromResponse(
                  response.data
                );
          ...
          .catch(error => {
            ...
            let srt_to_show = this.$t(
              pop_up_msg.instance.error.execute
            ).format([this.$t(opt.name), this.$t(this.view.schema.name), str]);
      ...
      getBreadcrumbParentPart() {
        let breadcrumb = [];
        let parent_links = this.getParentPaths(
          this.$route.name,
          this.$route.path
        );
        ...
        for (let index = 0; index < parent_links.length; index++) {
          ...
          if (this.data.parent_instances[parent_link.url]) {
            name_1 = this.data.parent_instances[
              parent_link.url
            ].getViewFieldValue();
          }
      ...
      getBreadcrumbCurrentPath() {
        ...
        if (filters.length > 0) {
          ...
          if (page) {
            obj.link =
              this.$route.path.replace(/\/$/g, "") + "?" + filters.join("&");
          }
  ...
  list: {
    ...
    methods: {
      ...
      setInstancesToData(instances) {
        ...
        this.data.pagination.count = this.getQuerySet(
          this.view,
          this.qs_url
        ).api_count;
        this.data.pagination.page_number =
          this.getFiltersPrepared(this.qs_url).page || 1;
      },
            ...
            if (id == instance.getPkValue()) {
              instance
                ...
                .catch(error => {
                  ...
                  let srt_to_show = this.$t(
                    pop_up_msg.instance.error.remove
                  ).format([
                    instance.getViewFieldValue(),
                    this.$t(this.view.schema.name),
                    str
      ...
      removeInstances_callback(instance, response) {
        ...
        let url =
          this.qs_url.replace(/^\/|\/$/g, "") + "/" + instance.getPkValue();
       ...
       */
      updateData() {
        let new_qs = this.getQuerySet(this.view, this.qs_url)
          .clone()
          .prefetch();
        return new_qs
          .items()
          .then(instances => {
            if (
              deepEqual(
                this.getQuerySet(this.view, this.qs_url).query,
                new_qs.query
              )
            ) {
              this.setQuerySet(this.view, this.qs_url, new_qs);
  ...
  page_new: {
    ...
    methods: {
      ...
      getRedirectUrl(opt) {
        return [
          this.url.replace("/edit", "").replace("/new", ""),
          opt.instance.getPkValue()
        ].join("/");
      }
    }
  },
  ...
  action: {
    ...
    methods: {
      ...
      _getRedirectUrlBase(opt) {
        ...
        let view = this.$store.getters.getView(
          parent_paths[parent_paths.length - 1].path
        );
