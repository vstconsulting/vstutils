...
runner.module('guiFields[field].{toInner, toRepresent}()');
 ...
 */
export function addTestForGuiFieldInstanceMethods(options, values) {
    runner.test(options.format, function (assert) {
...
let fields_constructor = {
        ...
        values: [
            {
                ...
                output: {
                    toInner:
                        '2019-05-08T07:08:07' +
                        (app.api.getTimeZone() === 'UTC'
                            ? 'Z'
                            : moment.tz(app.api.getTimeZone()).format('Z')),
                    toRepresent:
                        moment(moment.tz('2019-05-08T07:08:07', app.api.getTimeZone()))
                            .tz(moment.tz.guess())
                            .format('YYYY-MM-DD') +
                        'T' +
                        moment(moment.tz('2019-05-08T07:08:07', app.api.getTimeZone()))
    ...
    json: {
        options: { format: 'json', readOnly: true },
        values: [
            {
...
for (let key in fields_constructor) {
    ...
    addTestForGuiFieldInstanceMethods(window.$.extend(true, {}, base_opt, item.options), item.values);
}
