...
class GUITests {
     ...
     * @param {string} prefix Key prefix, at which data will be saved.
     */
    saveInstanceData(instances_info, is_parent, prefix = '') {
        ...
        instances_info.key_fields_data[prefix + view.schema.name][model.pk_name] = instance.getPkValue();
        instances_info.key_fields_data[prefix + view.schema.name][
            model.view_name
    ...
    openPage(path, params, expectation = true) {
        runner.test(`guiViews['${path}'].open`, function (assert) {
            let done = assert.async();
            let url = path.format(params).replace(/\/$/g, '');

            spa.utils.current_view._initLoadingPromise();

            if (url.replace(/^\/|\/$/g, '') === app.application.$route.path.replace(/^\/|\/$/g, '')) {
            ...
            let getTestResult = (result) => {
                assert.strictEqual(result, expectation, `guiViews['${path}'].opened`);
                done();
            };
    ...
    wait(test_name, time = undefined) {
        ...
        runner.test(`wait['${test_name}']`, function (assert) {
            let done = assert.async();
            setTimeout(() => {
     ...
     */
    click(selector, callback = undefined, fallback = undefined) {
        runner.test(`click on '${selector}' element`, function (assert) {
     ...
     * @param {string} selector Selector of element to click.
     * @param {boolean=true} expectation If true - test is expected to be successful. Otherwise, it's expected to be failed.
     * @param {function=} callback Callback for a successful redirect.
     * @param {function=} fallback Callback for a redirect with error.
     */
    clickAndWaitRedirect(selector, expectation = true, callback = undefined, fallback = undefined) {
        runner.test(`click on '${selector}' element and wait redirect`, function (assert) {
            let done = assert.async();
     ...
     */
    setValues(path, data) {
        runner.test(`guiViews['${path}'].setValues`, function (assert) {
    ...
    compareValues(path, data) {
        runner.test(`guiViews['${path}'].compareValues`, function (assert) {
            let view = app.views[path];
    ...
    checkViewButtons(path) {
        runner.test(`guiViews['${path}'].checkButtons`, function (assert) {
            let view = app.views[path];

            let operations = view.schema.operations;

            for (let btn of Object.values(operations)) {
                if (btn.hidden) {
                    continue;
                }
     ...
     * @param {string} path - Path of view.
     * @param {Object} instances_info.
     * @param {Object} instances_info.url_params - URL params for path.
     * @param {Object} instances_info.key_fields_data - view && values fields values of created
     * during test Model instances.
     * @param {boolean=false} is_parent
     */
    setFilterValues(path, instances_info, is_parent = true) {
        runner.test('guiViews[' + path + '].setFilterValues', function (assert) {
            let view = app.views[path];
    ...
    checkReloadButtonWork(path, data) {
        ...
        runner.test(`guiViews['${path}].checkReloadButton`, (assert) => {
    ...
    executeActionFromSomeView(path, instances_info, options, callback, fallback) {
        ...
        this.clickAndWaitRedirect('.btn-operation-execute', options.is_valid, callback, fallback);
    }
    /**
    ...
    testAddChildInstanceToParentList(path, instances_info, options) {
        ...
        this.wait('modal opening', spa.utils.guiLocalSettings.get('guiApi.real_query_timeout') * 3);

        runner.test(`guiViews['${path}'].addChild set filter value`, function (assert) {
            let view = app.views[options.child_path];

            const searchInput = document.getElementsByClassName('modal-list-search-input')[0];
            assert.ok(searchInput !== undefined);

            searchInput.value =
                instances_info.key_fields_data['child_' + view.schema.name][view.objects.model.view_name];
            searchInput.dispatchEvent(new Event('input'));

            document.querySelector('.modal-list-search-apply').click();
        });

        this.wait('list filtering', spa.utils.guiLocalSettings.get('guiApi.real_query_timeout') * 3);

        runner.test('guiViews[' + path + '].addChild select and add child instance', function (assert) {
            assert.ok($('.modal-body .item-row').length > 0);

            $('.modal-body .item-row .td_select_btn')[0].dispatchEvent(new Event('click'));
            $('.btn-operation-add-child-apply')[0].dispatchEvent(new Event('click'));
        });
        ...
        runner.test(`guiViews['${path}'].addChild check instance was added`, function (assert) {
            let done = assert.async();
            let id = instances_info.url_params[`child_${view.schema.name}_id`];

            const itemRow = $(`tr.item-row[data-id=${id}]`);

            assert.ok($('.item-row').length > 0);
            assert.ok(itemRow.length > 0);
        ...
        this.testRemovePageViewInstance(options.child_path + '{' + pk_key + '}/', instances_info, true);
    }
    /**
     ...
     */
    testListViewFilters(path, instances_info, expectation = true, is_parent = true, open_by_path = true) {
        if (open_by_path) {
            this.openPage(path, instances_info.url_params);
        }
     ...
     * (it's supposed, that test is following after 'page_new' or 'page_edit' type view).
     */
    testListViewSelections(path, instances_info, expectation = true, open_by_path = true) {
        if (open_by_path) {
            this.openPage(path, instances_info.url_params);
        }
        ...
        this.click('.global-select.td_select_btn', (assert) => {
            const done = assert.async();
            setTimeout(() => {
                const numOfSelectedRows = $('.item-row.selected').length;
                assert.ok(numOfSelectedRows > 0 === expectation);
                assert.ok(($('tr.selected').length - numOfSelectedRows === 1) === expectation);
                compareSelectionsWithStore(assert);
                done();
            }, 20);
        });
     ...
     */
    testPageNewView(path, instances_info, options, is_parent = true, open_by_path = true) {
        if (open_by_path) {
            this.openPage(path, instances_info.url_params);
        } else {
        ...
        // eslint-disable-next-line no-unused-vars
        this.clickAndWaitRedirect('.btn-operation-save', options.is_valid, (assert) => {
            if (!options.is_valid) {
                return;
            }
    ...
    testSetOfViews(list_path, instances_info, test_options, is_parent) {
        ...
        for (let [idx, val] of test_options.new.entries()) {
            this.testPageNewView(list_path + 'new/', instances_info, val, is_parent, idx !== 0);
        }

        this.testPageView(list_path + '{' + pk_key + '}/', instances_info, false);

        for (let [idx, val] of test_options.edit.entries()) {
            let path_edit = list_path + '{' + pk_key + '}/' + 'edit/';
            this.testPageEditView(path_edit, instances_info, val, idx !== 0);
        }
        ...
        if (test_options.page && test_options.page.remove) {
            this.testRemovePageViewInstance(list_path + '{' + pk_key + '}/', instances_info, true);
        }
    }
}
 ...
 */
const guiTests = new GUITests();

export { GUITests, guiTests };
