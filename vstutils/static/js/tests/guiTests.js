...
class GuiTests {
    ...
    saveInstanceData(instances_info, is_parent, prefix="") {
        ...
        instances_info.key_fields_data[prefix + view.schema.name][model.view_name] = instance.getViewFieldValue();
    }
    /**
    ...
    clickAndWaitRedirect(selector, expectation=true, callback, fallback) {
        syncQUnit.addTest("click on '" + selector + "' element and wait redirect", function(assert) {
            let done = assert.async();
    ...
    compareValues(path, data) {
        syncQUnit.addTest("guiViews[" + path + "].compareValues", function(assert) {
            let done = assert.async();
            let view = app.views[path];
            ...
            for(let key in test_data) {
                ...
                if(typeof test_data[key].value == 'object') {
                    bool = deepEqual(test_data[key].value, store_data[key]);
                } else {
    ...
    checkViewButtons(path) {
        syncQUnit.addTest("guiViews[" + path + "].checkButtons", function(assert) {
            ...
            for(let key in operations) {
                ...
                assert.ok($(".btn-operation-" + btn_name).length > 0, "guiViews[" + path + "].checkButtons - " + btn_name);
            }
    ...
    setFilterValues(path, instances_info, is_parent=true) {
        syncQUnit.addTest("guiViews[" + path + "].setFilterValues", function(assert) {
            let done = assert.async();
            let view = app.views[path];
    ...
    checkReloadButtonWork(path, data) {
        ...
        syncQUnit.addTest("guiViews[" + path + "].checkReloadButton", function(assert) {
            ...
            assert.ok(deepEqual(api_data, edit_data), 'data was reloaded');
    ...
    testAddChildInstanceToParentList(path, instances_info, options) {
        ...
        this.wait("modal opening", guiLocalSettings.get('guiApi.real_query_timeout') * 3);

        syncQUnit.addTest("guiViews[" + path + "].addChild set filter value", function(assert) {
            let done = assert.async();
            let view = app.views[options.child_path];

            assert.ok($(".modal-list-search-input").length > 0);

            let filter_value = instances_info.key_fields_data['child_' + view.schema.name][view.objects.model.view_name];
        ...
        this.wait("list filtering", guiLocalSettings.get('guiApi.real_query_timeout') * 3);

        syncQUnit.addTest("guiViews[" + path + "].addChild select and add child instance", function(assert) {
            let done = assert.async();
            let view = app.views[options.child_path];
        ...
        syncQUnit.addTest("guiViews[" + path + "].addChild check instance was added", function(assert) {
            let done = assert.async();
    ...
    testListViewSelections(path, instances_info, expectation=true, open_by_path=true) {
        ...
        this.click(".global-select.td_select_btn", (assert) => {
            setTimeout(() => {
                assert.ok(($(".item-row.selected").length > 0) == expectation);
                assert.ok((($("tr.selected").length - $(".item-row.selected").length) == 1) == expectation);
                compareSelectionsWithStore(assert);
            }, 20);
        });
    ...
    testSetOfViews(list_path, instances_info, test_options, is_parent) {
        ...
        for(let index in test_options.new) {
            this.testPageNewView(list_path + "new/", instances_info, test_options.new[index], is_parent, index != 0);
        }
        ...
        for(let index in test_options.edit) {
            ...
            this.testPageEditView(path_edit, instances_info, test_options.edit[index], index != 0);
        }
