...
fn decode(key: &str, enc: &str) -> PyResult<String> {
    let enc_bytes = general_purpose::URL_SAFE.decode(enc).map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    ...
    let decoded = String::from_utf8(dec).map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    Ok(decoded)
}

/// Define the Python module.
#[pymodule]
fn _utils(py: Python, m: &PyModule) -> PyResult<()> {
